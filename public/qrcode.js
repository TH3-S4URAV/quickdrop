/**
 * Standalone QR Code Generator for QuickDrop
 * Self-contained, zero external network requests, works 100% offline.
 * Based on standard ISO/IEC 18004 QR Code specification.
 */
(function (root, factory) {
    ...
    getRSBlocks: function (typeNumber, errorCorrectLevel) {
      ...
      for (let i = 0; i < length; i++) {
        ...
        for (let j = 0; j < count; j++) {
          list.push(new QRRSBlockItem(totalCount, dataCount));
        }
      }
      return list;
    },
    ...
    make: function () {
      if (this.typeNumber < 1) {
        ...
        for (typeNumber = 1; typeNumber < 14; typeNumber++) {
          ...
          let totalData = 0;
          for (let i = 0; i < rsBlocks.length; i++) totalData += rsBlocks[i].dataCount;
  ...
  function QRCode(target, vOption) {
    ...
    if (typeof vOption === 'string') {
      ...
      for (const i in vOption) this._htOption[i] = vOption[i];
    }
  ...
  QRCode.prototype.render = function () {
    if (!this._el) return;
    this._el.innerHTML = "";
    const moduleCount = this._oQRCode.getModuleCount();
    const width = this._htOption.width;
    const height = this._htOption.height;

    // Use Canvas if available, otherwise SVG
